...
/*
this program is to replace the empty snps in input file with first non-empty SNPs in each chromosone
...
Also have to change array pairs[22] to the base pairs of first non empty snp in each chromosone
...
int main(){
	...
	string suffix="txt";
	string suffix2="csv";
	string outputfile="Same_Min4"+inputfile+"."+suffix;
			...
			while (true){
				...
				if (i==3){
					...
					if (data=="--" && j<min-exist[chrom-1]){
						data=pairs[chrom-1];//replace the empty pair with random one
						j++;
					}
					line=line+data;
